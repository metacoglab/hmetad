...
The resulting dataframe has three columns: `N_0` is the number of trials with `stimulus==0`, `N_1` is the number of trials with `stimulus==1`, and `N` is a matrix containing the number of joint responses for each of the two possible stimuli (with column names indicating the `stimulus` and `joint_response`).
...
### Parameter estimates
First, it is often useful to extract the posterior draws of the model parameters. Here we can use `tidybayes::linpred_draws` (with arguments `dpar=TRUE` and `transform=TRUE` to extract the estimates of all model parameters on their true scale):
```{r parameters}
...
print(draws.linpred)
```
This `tibble` has a separate row for every posterior sample and a separate column for every model parameter (`.linpred` and `mu` both representing $M\textrm{-ratio}$). This format is useful for some purposes, but it will often be useful to pivot it so that we have a separate row for each model parameter and posterior sample:
```{r}
...
### Metacognitive bias
While mean confidence is often empirically informative, it is not recommended as a measure of metacognitive bias because it is known to be confounded by type 1 response characteristics (i.e., \eqn{d'} and \eqn{c}) and by metacognitive sensitivity (i.e., \eqn{\textrm{meta-}d'}{meta-d'}). Instead, we recommend a new measure of metacognitive bias, \eqn{\textrm{meta-}\Delta}{meta-delta}, which is the distance between the average of the confidence criteria and \eqn{\textrm{meta-}c}{meta-c}. 

\eqn{\textrm{meta-}\Delta}{meta-delta} can be interpreted as lying between two extremes: when \eqn{\textrm{meta-}\Delta = 0}{meta-delta}, the observer only uses the highest confidence rating, and when \eqn{\textrm{meta-}\Delta = \infty}{meta-delta}, the observer only uses the lowest confidence rating. 

To obtain estimates of \eqn{\textrm{meta-}\Delta}{meta-delta}, one can use the function `metacognitive_bias_draws`:
