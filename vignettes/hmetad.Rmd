...
```{r model_fitting, results=FALSE, warning=FALSE, error=FALSE, message=FALSE}
m <- fit_metad(N ~ 1,
  data = d, init = 0,
...
Alternatively, if the only issue is with the automatic data aggregation, one can provide the argument `aggregate=FALSE` to `fit_metad`:
...
As a measure of performance, one can similarly obtain the area under this curve:
```{r auroc1}
tibble(.row = 1) |>
  add_auroc1_draws(m) |>
  median_qi() |>
