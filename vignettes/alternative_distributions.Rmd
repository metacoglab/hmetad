---
title: "Using alternative distributions for the meta-d' model"
output: rmarkdown::html_vignette
vignette: >
  %\VignetteIndexEntry{alternative_distributions}
...
* `<distribution>_lcdf(x, mu)`: the log cumulative distribution function defining \eqn{\textrm{log} P(X \le x \;\vert\; mu)} for a random variable \eqn{X}.
* `<distribution>_lccdf(x, mu)`: the log complementary cumulative distribution function defining \eqn{\textrm{log} P(X \ge x \;\vert\; mu)} for a random variable \eqn{X}.
...
First, the `distribution` argument should be the name of the distribution as a string. This should be the part of the function names preceding `"_lcdf"` and `"_lccdf"` in both `R` and in `Stan`.

Second, the `stanvars` argument should be the `stanvar` object you have created above containing the `Stan` code for your cumulative distribution functions.
...
The model summary can be interpreted just as with any other model, however here you can see that the model family is `metad__4__gumbel_min__absolute`, indicating that this model indeed uses the `gumbel_min` distribution with four confidence levels and \eqn{\textrm{meta-}c = c}. 
